...
class TestHistoricalRetrieval:
    ...
    def test_get_history_multiple_snapshots(self, store):
        ...
        assert history[0]['mentions'] == 100  # Oldest
        assert history[-1]['mentions'] == 140  # Newest
    ...
    def test_get_history_filters_by_time(self, store):
        ...
        old_snapshot = TickerNarrativeSnapshot(
            ticker="BTC",
            window="4h",
            total_mentions=50,
            mindshare_score=0.1,
...
class TestVelocityCalculation:
    ...
    def test_calculate_velocity_mindshare(self, store):
        ...
        assert velocity['mindshare_delta'] == 0.05
...
class TestAnomalyDetection:
    ...
    def test_detect_anomalies_spike(self, store):
        ...
        # Insert 11 normal snapshots
        for i in range(11):
            snapshot = TickerNarrativeSnapshot(
                ticker="BTC",
                window="4h",
                total_mentions=100,  # Consistent
                mindshare_score=0.1,
                top_smart_accounts=[],
                source_query=f"query_{i}",
                timestamp=base_time - timedelta(hours=24-i*2)
        ...
        spike_snapshot = TickerNarrativeSnapshot(
            ticker="BTC",
            window="4h",
            total_mentions=300,  # Large spike
            mindshare_score=0.3,
            top_smart_accounts=[],
            source_query="spike",
            timestamp=base_time
    ...
    def test_detect_anomalies_drop(self, store):
        ...
        drop_snapshot = TickerNarrativeSnapshot(
            ticker="BTC",
            window="4h",
            total_mentions=20,  # Large drop
    ...
    def test_detect_anomalies_severity(self, store):
        ...
        extreme_snapshot = TickerNarrativeSnapshot(
            ticker="BTC",
            window="4h",
            total_mentions=500,  # Extreme spike
            mindshare_score=0.5,
            top_smart_accounts=[],
...
class TestCleanup:
    ...
    def test_cleanup_old_data(self, store):
        ...
        recent_snapshot = TickerNarrativeSnapshot(
            ticker="BTC",
            window="4h",
            total_mentions=100,
            mindshare_score=0.15,
